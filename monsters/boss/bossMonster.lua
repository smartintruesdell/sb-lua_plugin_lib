...
local PLUGINS_PATH =
  "/monsters/boss/bossMonster_plugins.config"
...
function init()
  ...
  --Non-combat states
  local states = stateMachine.scanScripts(
    config.getParameter("scripts"),
    "(%a+State)%.lua"
  )
...
function update(dt)
  ...
  if not status.resourcePositive("health") then
    ...
    trackTargets(
      self.keepTargetInSight,
      self.queryTargetDistance,
      self.trackTargetDistance,
      self.switchTargetDistance
    )

    for skillName, _params in pairs(self.skillParameters) do
...
function update_handle_death(self, _dt)
  self.state.endState()
  self.dead = true
end
