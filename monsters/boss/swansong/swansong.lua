...
local PLUGINS_PATH =
  "/monsters/boss/swansong/swansong_plugins.config"
...
init = PluginLoader.add_plugin_loader("swansong", PLUGINS_PATH, init)
...
function breakAdds()
  for _, entityId in ipairs(queryOphanims()) do
...
meleeDashAttack = async(function(conf, targetAngle)
  ...
  await(join(
    function()
      while vec2.mag(mcontroller.velocity()) > 1.0 do
