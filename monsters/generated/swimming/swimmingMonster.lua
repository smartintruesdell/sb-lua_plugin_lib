...
local PLUGINS_PATH =
  "/monsters/generated/swimming/swimmingMonster_plugins.config"
...
init = PluginLoader.add_plugin_loader("swimmingMonster", PLUGINS_PATH, init)
...
function collides(sensorGroup, direction)
  for i, sensor in ipairs(config.getParameter(sensorGroup)) do
    -- world.debugPoint(monster.toAbsolutePosition(sensor), "blue")
    if world.pointTileCollision(monster.toAbsolutePosition(sensor), {"Dynamic", "Null", "Block"}) then
      return true
    end
  end
