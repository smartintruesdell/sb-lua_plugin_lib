...
local PLUGINS_PATH =
  "/monsters/bugs/bug_plugins.config"
...
init = PluginLoader.add_plugin_loader("bug", PLUGINS_PATH, init)
...
function update(dt)
  ...
  if world.lineTileCollision(position, minGroundLine, {"Null", "Block", "Dynamic", "Slippery"}) or world.liquidAt(minGroundLine) then
    self.direction[2] = self.direction[2] + self.riseSpeed * dt
  elseif not world.lineTileCollision(position, maxGroundLine, {"Null", "Block", "Dynamic", "Slippery"}) or not world.liquidAt(maxGroundLine) then
  ...
  mcontroller.controlFly(self.direction)
