...
function init()
  ...
  if self.podUuid == nil then
    self.podUuid = sb.makeUuid()
    activeItem.setInstanceValue("podUuid", self.podUuid)
  end
...
init = PluginLoader.add_plugin_loader("filledcapturepod", PLUGINS_PATH, init)
...
function finishInit()
  ...
  -- 'pets' is what they are restored to when the filledcapturepod is healed.
